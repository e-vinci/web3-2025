   ...
   export async function generateExpenseReport(data: ExpenseReportData): Promise<string> {
     ...
     const expenses = await prisma.expense.findMany({
       where: {
         ...
         ...(data.endDate && { date: { lte: data.endDate } }),
       },
       include: {
         payer: true,
         participants: true,
     ...
     return new Promise((resolve, reject) => {
       ...
       if (data.startDate || data.endDate) {
         doc.text(
           `Period: ${data.startDate?.toLocaleDateString() || 'All'} - ${data.endDate?.toLocaleDateString() || 'All'}`
         );
         doc.moveDown();
       }
...
2. **Add GraphQL types and resolvers** in `src/api/report/augmentGraphqlSchema.ts`:
   ...
   const augmentSchema = (builder: typeof SchemaBuilder) => {
     ...
     const ReportJobRef = builder.objectType('ReportJob', {
       fields: (t) => ({
         ...
         downloadUrl: t.string({
           ...
           resolve: (parent) => {
             if (parent.status === 'completed' && parent.result) {
               return `/reports/${parent.result.filePath.split('/').pop()}`;
             }
             return null;
           },
         }),
...
### 6. Connect Socket.io Client in React
...
### 7. Broadcast Expense Creation Events
   ...
   pdfWorker.on('completed', (job) => {
     ...
     if (job.returnvalue) {
       emitReportReady({
         ...
         userId: job.data.userId,
         downloadUrl: `/reports/${job.returnvalue.filePath.split('/').pop()}`,
       });
     }
   });
   ```
...
### 8. Add Real-time Notifications UI
...
### 9. (Bonus) Add Scheduled Jobs for Monthly Reports
