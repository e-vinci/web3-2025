...
**Goal**: Get our work (backend & frontend) ready to deploy
...
Once built, the font is "just" standard html & js - so it is indeed a "static website".
...
- The install Prisma and run the setup:
...
(be careful to run this in the proper folder, ie the backend)

- Replace the DATABASE_URL in the .env file with the one from render (or your local one)
...
The import piece is the main function and especially:
...
- As a first step, create a "handleSubmit" method to be called on submit, outputing (via the console or an alert) the form contant
